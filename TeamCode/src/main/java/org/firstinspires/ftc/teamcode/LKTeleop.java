...
@TeleOp(name="LK Tele-Op", group ="Real")
public class LKTeleop extends LinearOpMode {
    ...
    @Override public void runOpMode() {
        ...
        try {
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
                DLog = new DataLogger(LocalDateTime.now() + "dlog.csv");
            }
        } catch (IOException e) {
            e.printStackTrace();
        }
        ...
        public void run()
        {
            try
            {
                while (!isInterrupted())
                {
                    ...
                    double curr = Shooter.getCurrent(CurrentUnit.AMPS);
                    telemetry.addData("time", runtime);
                    DLog.addHeaderLine(runtime.toString() + "," + speed + "," + curr );
